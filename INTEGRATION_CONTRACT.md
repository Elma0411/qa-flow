...
- `retrieval_trace` is optional on old items. New primary QA items should carry
  ...
  include `retrieval_query`, `must_have_terms`, `answer_scope`,
  `retrieval_mode`, `semantic_top_k`, `rerank_top_n`, hybrid weights,
  `raw_semantic_hits`, and `selected_evidence`.
- Each `raw_semantic_hits` record may include `score`, `dense_score`,
  ...
  `same_parent`, `adjacent`, and `title_overlap`.
