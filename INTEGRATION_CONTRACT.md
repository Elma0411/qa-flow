...
- Task status and consolidated timing expose `unit_plan_summary` for the final
  document-level allocation. Its stable counters are
...
- `qa_evaluation_evidence_text` is the readable subset of evidence blocks that
  ...
  evaluation metrics. It may contain `正文证据-N`, `图片证据-N`, or cited
  supplemental evidence blocks. Legacy items without it fall back to
  `qa_generation_unit_text` and then older source fields.
...
- `llm_evidence_text_by_ref` is the paired ephemeral label-to-rendered-text
...
- `retrieval_trace` is optional on old items. New primary QA items should carry
  ...
  `selected_evidence_chunk_ids`, `selected_evidence_window_count`,
  `selected_evidence_chunk_count`, `final_evidence_k`,
  `evidence_token_budget`, and `evidence_tokens_estimated`.
- `final_evidence_k` is the maximum supplemental-window count. The actual
  count is `selected_evidence_window_count` (or the length of
  `selected_windows`) and may be `0` after relevance admission.
- Chunk QA inspection must match both the scalar `source_chunk_id` and the
  complete `source_chunk_ids` set. A multi-material Summary QA therefore
  appears when any of its primary source chunks is opened; the scalar source
  remains only the first directly cited primary chunk.
