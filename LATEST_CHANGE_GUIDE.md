...
- UI 资源与页面导航改用相对 `/ui/` 路径，并根据当前 URL 自动推断 API 代理前缀，兼容直连、SSLVPN Web 代理和 WebVPN 前缀访问；流水线脚本版本标记同步为 `2026-08-15-3`。
- `/ui/*.html` 增加 `no-store/no-cache` 响应头，避免 VPN 代理缓存旧 HTML 后与新版 JS/CSS 混合加载。
